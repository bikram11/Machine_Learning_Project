...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in tqdm(dataset):
        ...
        if not os.path.exists(os.path.join(opt.features, imagename +'.pt')) :
            img = torch.from_numpy(img).to(device)
            img = img.half() if half else img.float()  # uint8 to fp16/32
            img /= 255.0  # 0 - 255 to 0.0 - 1.0f
            if img.ndimension() == 3:
                img = img.unsqueeze(0)
            ...
            model.model[100].register_forward_hook(get_activation('after22')) # 22 is before last BottleneckCSP

            pred = model(img, augment=opt.augment)[0]

            # save extracted features
            if not os.path.exists(opt.features):
                os.makedirs(opt.features)
            # imagename = (path.split('/')[-1]).split('.')[0]
